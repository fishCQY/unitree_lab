...
class CircularBuffer:
    ...
    def mini_batch_generator(self, fetch_length: int, num_mini_batches: int, num_epochs=8):
        ...
        Each epoch re-samples from the full buffer to maximize data diversity
        ...
        for epoch in range(num_epochs):
            ...
            perm = torch.randperm(epoch_batch_size, requires_grad=False, device=self.device)
            for i in range(num_mini_batches):
                start = i * mini_batch_size
                end = (i + 1) * mini_batch_size
                mini_indices_0 = indices_0[perm[start:end]]
                mini_indices_1 = indices_1[perm[start:end]]
